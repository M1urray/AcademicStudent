    ...
    public class DashboardController : Controller
    {
                ...
                else
                {
                    ...
                    using (var streamReader = new StreamReader(httpResponseResC.GetResponseStream()))
                    {
                        ...
                        foreach (JObject config in details["value"])
                        {
                            ...
                            Details.AttemptedUnits = AttempUnits;// (string)config["Completed_Units"];
                            ...
                            Details.Qualify_For_Catering = true;//onfig["Tax_Liable"];
        ...
        public PartialViewResult MakePayment()
        {
            try
            {
                ...
                string pageResC = "StudentServices?$filter=Category eq 'Student'&$format=json";
                ...
                using (var streamReader = new StreamReader(httpResponseC.GetResponseStream()))
                {
                    ...
                    foreach (JObject config in details["value"])
                    {
                        ...
                        d.ServiceCode = (string)config["Service_Code"];
                        d.ServiceDesc = (string)config["Service_Description"];
                        servicesList.Add(d);
                    }
                }
                ...
                {
                    ListofServices = servicesList.Select(x =>
                                          new SelectListItem()
                                          {
                                              Text = x.ServiceDesc,
                                              Value = x.ServiceCode
                ...
                return PartialView("~/Views/Dashboard/MakePaymentForm.cshtml", payments);
            }
            catch (Exception ex)
            {
                Error error = new Error();
                error.Message = ex.Message.Replace("'", "");
                return PartialView("~/Views/Common/ErrorMessage.cshtml", error);
            }
        }
    }
}