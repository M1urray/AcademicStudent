    ...
    public class WelfareController : Controller
    {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (r[0] == "")
                            {
                                 Credentials.ObjNav.StudentSelfPromotion(RegNo, Prog, "");
                                r = CommonClass.CurrentCourseRegistration(RegNo, sem, "0");
                            }
                                                ...
                                                if (gender != "")
                                                {
                                                    ...
                                                    using (var streamReader = new StreamReader(httpResponseResC.GetResponseStream()))
                                                    {
                                                        ...
                                                        foreach (JObject config in details["value"])
                                                        {
                                                            ...
                                                            Hlist.Cost = ((decimal)config["Cost_Per_Occupant"]).ToString("#,##0.00");
        ...
        public JsonResult SaveSelectedSpace(RoomSpaces spaceDetails)
        {
            try
            {
                ...
                Credentials.ObjNav.GenerateHostelAllocationWithPayment(StdNo, sem, spaceDetails.HostelCode, spaceDetails.RoomCode, spaceDetails.SpaceCode, Convert.ToDecimal(spaceDetails.Cost));
